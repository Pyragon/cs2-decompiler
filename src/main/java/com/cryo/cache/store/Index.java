...
import com.cryo.cache.store.util.Constants;
...
public final class Index {
	...
	public boolean putFile(int archiveId, int fileId, byte[] data) {
		return putFile(archiveId, fileId, Constants.GZIP_COMPRESSION, data, null, false, false, -1, -1);
	}
		...
		if (filesCount == 1)
			...
			for (int i = 0; i < filesCount; i++) {
				try {
					...
					e.printStackTrace();
					System.out.println("Error with id: "+archiveId);
					return;
				}
			}
			stream.setOffset(readPosition);
