...
public final class InputStream extends Stream {
    ...
    public int readBits(int bitOffset) {
        ...
        if ((i_8_ ^ 0xffffffff) == (bitOffset ^ 0xffffffff)) value += buffer[bytePos] & BIT_MASK[i_8_];
        else value += (buffer[bytePos] >> -bitOffset + i_8_ & BIT_MASK[bitOffset]);
        return value;
    }
    ...
    public int read_tri_byte() {
        int i = (readUnsignedByte() << 16) + (readUnsignedByte() << 8) + readUnsignedByte();
        return i;
    }
