...
public class OutputStream extends Stream {
    ...
    public void checkCapacityPosition(int position) {
        if (position >= getBuffer().length) {
            byte[] newBuffer = new byte[position + 1];
    ...
    public void writeBigSmart(int i) {
        if (i >= Short.MAX_VALUE) writeInt(i - Integer.MAX_VALUE - 1);
        else {
    ...
    public void writeJagString(String string) {
        checkCapacityPosition(getOffset() + string.length() + 2);
        writeByte(0);
        System.arraycopy(string.getBytes(), 0, getBuffer(), getOffset(), string.length());
        setOffset(getOffset() + string.length());
        writeByte(0);
    }
